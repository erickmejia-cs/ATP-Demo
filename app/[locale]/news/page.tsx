...
export async function generateMetadata({
  ...
  const newsPage = await getLatestNewsPage(params.locale);
  return {
  ...
  return (
    <>
      ...
      <div className="news-listing-header">
        <h1
          className="news-listing-title"
          {...cslpAttr(entryUid, locale, 'title')}
        >
        ...
        {newsPage?.intro && (
          <p
            className="news-listing-intro"
            {...cslpAttr(entryUid, locale, 'intro')}
          >
            {newsPage.intro}
          </p>
        )}
      ...
      {featuredArticles.length > 0 && (
        <section className="news-listing-section">
          <div
            className="news-grid"
            {...cslpAttr(entryUid, locale, 'featured_articles')}
          >
      ...
      <section className="news-listing-section">
        ...
        {totalPages > 1 && (
          <nav className="news-pagination" aria-label="Pagination">
            {safePage > 1 && (
              <Link
                href={`/${locale}/news?page=${safePage - 1}`}
